...
//prepare the query
$sql = "SELECT * FROM users_db WHERE admin = 1;";
$cmd = $conn -> prepare($sql);
// run the query and store the results
$cmd -> execute();
$users = $cmd -> fetchAll();
//start the grid with html
echo '<table class="table table-striped"><thead><th>User ID</th><th>Username</th><th>email</th><th>Edit</th><th>Delete</th></thead><tbody>';
